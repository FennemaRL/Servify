...
import { Button, Tabs, Cascader } from 'antd';
import axios from "axios";
import FormEditService from "./formService";
const { TabPane } = Tabs;
...
function ViewEditableService ({ username, services, setServices }) {
    ...
    const add = () => {//hacer el add de la api
        ...
        if (selectCategory[0] ){
          ...
          axios.post(`${process.env.REACT_APP_API_URL}/api/provider/service`,{ username:username,category:selectCategory[0]})
    ...
    const remove = targetKey =>  {
            ...
            axios.delete(`${process.env.REACT_APP_API_URL}/api/provider/service`,{data:{ username:username, category:targetKey}})
